...
public class Grammar2012 extends AbstractGrammar {
	...
	private void buildReducemap() {
				...
				if (keys.length() > 0) {
					...
					if (order >= reducemap.size()) {
						for (int i = reducemap.size(); i <= order; ++i) {
							reducemap
									.add(new MultiMap<String, Pair<Entity, Entities>>());
							reducekeys.add(new HashSet<String>());
						}
					}
					reducemap.get(order).multiPut(keys,
							new Pair<Entity, Entities>(target, it.getValue()));
		...
		for (int i = 0; i < reducemap.size(); ++i) {
			...
			Collections.sort(reducekeysSorted.get(i),
	...
	private List<Entities> expandEntry(Entities oldvalue,
			Mutable<Integer> expanded, Mutable<Boolean> complete) {
		...
		for (final Entity toReplace : oldvalue) {
			...
			if (toReplace.type() == 'r') {
				...
				if (count == 0) {
					LogUtils.e("Error! Count of symbol is 0: "
							+ toReplace.toString());
				} else if (count == 1) {
					Entities replacement = grammarmap.get(keyOfToReplace)
							.iterator().next();
					...
					if (newvalues.size() == 1) {
						...
						Set<Entities> replacements = grammarmap
								.get(keyOfToReplace);
						g = 0;
						for (Entities replacement : replacements) {
							newvalues.get(g++)
									.add(replacement, toReplace, this);
						}
					} else {
	...
	private List<Entities> reduce(Entities oldWordsI) {
		...
		int step = 0;
		while (step++ <= 50) {
			LogUtils.i("reduce step " + step + "... " + wordsList.size()
					+ " possibilities...");
			if (wordsList.size() == 0)
				break;
	...
	private HashMap<String, Entities> reduceStep(Entities oldWordsI) {
		...
		while (!found && ++order < reducemap.size()) {
			...
			for (final String key : reducekeysSorted.get(order)) {
				...
				if (oldImpression.contains(key)) {
					...
					for (final Pair<Entity, Entities> rule : reducemap.get(
							order).get(key)) {
						...
						if (complexityKey.contains("$")) {
							complexityKey = "{"
									+ complexityKey.substring(complexityKey
											.indexOf("$"));
						}
						...
						if (newWordsComplexityMap.containsKey(complexityKey)) {
							best_complexity = newWordsComplexityMap.get(
									complexityKey).getKey();
						}
						if (complexity >= best_complexity) {
							if (complexity > best_complexity) {
								newWordsComplexityMap.remove(complexityKey);
								newWordsComplexityMap
										.put(

										complexityKey,
												new AbstractMap.SimpleEntry<Integer, HashMap<String, Entities>>(
														complexity,
														new HashMap<String, Entities>()));
								inThisStepReduceTo = complexityKey;
								found = true;

								LogUtils.d("      best complexity. deleting worse data. ("
										+ complexity
										+ " > "
										+ best_complexity
										+ ")");
							} else {
								LogUtils.d("      equal complexity. ("
										+ complexity + " = " + best_complexity
										+ ")");

							}

							Entities newWordsI = this.replaceInList(oldWordsI,
									rule.getSecond(), rule.getFirst());

							newWordsComplexityMap.get(complexityKey).getValue()
									.put(

									this.allToString(newWordsI), newWordsI);
							if (complexityKey.contains("*"))
								newWordsComplexityMap.get(complexityKey)
										.getValue().put(

										this.allToString(oldWordsI), oldWordsI);

						} else {
							LogUtils.d("      too low complexity. ("
									+ complexity + " < " + best_complexity
									+ ")");
		...
		for (Map.Entry<Integer, HashMap<String, Entities>> entry : newWordsComplexityMap
				.values()) {
	...
	private Entities replaceInList(Entities vec, Entities find,
			Entity replacement) {
		if (find.size() == 0)
		...
		LogUtils.d("      replace " + print(find) + " with "
				+ print(replacement) + "\n      in " + print(vec)
				+ "\n      is " + print(newList) + "\n");
